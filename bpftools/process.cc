...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
    ...
    if (e->exit_event) {
        NANO_LOG(NOTICE, "%-8s %-5s %-16s %-7d %-7d [%u]",
               ts, "EXIT", e->comm, e->pid, e->ppid, e->exit_code);
        if (e->duration_ns) {
            NANO_LOG(NOTICE, " (%llums)", e->duration_ns / 1000000);
        }
    } else {
...
int main(int argc, char **argv)
{
    ...
    /*err = argp_parse(&argp, argc, argv, 0, NULL, NULL);// 解析字符串,这些字符串来源于命令行,属于用户态的范围.
    if (err)
        return err;*/
...
// 关于用户态程序的流程:open,load,attach
// new出来一个buffer,轮询这个buffer
